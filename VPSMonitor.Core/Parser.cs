    ...
    public static class Parser
    {
        ...
        public static async Task<NetworkInfo> ParseNetworkInfoAsync(string ipAddressCommandOutput, string gatewayCommandOutput, string gatewayIpV6CommandOutput)
        {
            ...
            return new NetworkInfo()
            {
                ...
                {
                    IpAddress = ipAddress,
                    Gateway = await Task.Run(() => ExtractGateway(gatewayCommandOutput)),
                    Netmask = await Task.Run(() => ExtractNetmask(ipAddressCommandOutput, ipAddress))
                },
                IpV6NetworkInfo = new IpV6NetworkInfo()
                {
                    IpAddress = parsedIpV6[0],
                    Gateway = await Task.Run(() => ExtractGatewayIpv6(gatewayIpV6CommandOutput)),
                    Netmask = $"/{parsedIpV6[1]}"
                }
            };
        }
